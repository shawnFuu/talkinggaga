...
class DiffTalkingHead(nn.Module):
    def __init__(self, args, device='cuda'):
        ...
        if self.audio_model == 'wav2vec2':
            ...
            try:
                ...
                spec = importlib.util.spec_from_file_location("HubertModel", "/home/fuxiaowen/3DGS/DiffPoseTalk/models/hubert.py")
                hubert_module = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(hubert_module)
                HubertModel = hubert_module.HubertModel
    ...
    def sample(self, audio_or_feat, shape_feat, style_feat=None, prev_motion_feat=None, prev_audio_feat=None,
        ...
        if shape_feat.ndim == 2:
            shape_feat = shape_feat.unsqueeze(1)  # (N, 1, d_shape) (3,1,100)
        if style_feat is not None and style_feat.ndim == 2:
            style_feat = style_feat.unsqueeze(1)  # (N, 1, d_style) (3,1,128)

        if prev_motion_feat is None:
            prev_motion_feat = self.start_motion_feat.expand(batch_size, -1, -1)  # (N, n_prev_motions, d_motion)，可学习参数
        if prev_audio_feat is None:
            # (N, n_prev_motions, feature_dim)
            prev_audio_feat = self.start_audio_feat.expand(batch_size, -1, -1)
        ...
        for t in tqdm(range(self.diffusion_sched.num_steps, 0, -1), desc="Diffusion Progress"):
            if t > 1:
            ...
            # Classifier-free Guidance (optional) cfg控制
        ...
        if ret_traj:
            return traj, motion_at_T, audio_feat # motion_at_T [3, 100, 54] 
        else:
