...
from utils.common import NullableArgs, coef_dict_to_vertices, get_coef_dict
from utils.media import combine_video_and_audio, convert_video, reencode_audio
from models.diff_talking_head import DiffTalkingHead
...
class Demo:
    def __init__(self, args, load_flame=True, load_renderer=True):
        ...
        if self.load_renderer:
            ...
            try:
                ...
                self.uv_coords = np.load('/data/fuxiaowen/talkinggaga/DiffPoseTalk/models/data/uv_coords.npz')
            self.size = (640, 640)
    ...
    def infer_from_file(self, audio_path, coef_path, out_path, style_path=None, tex_path=None, n_repetitions=1,
        ...
        coef_dict = self.infer_coeffs(audio_path, coef_path, style_path, n_repetitions,
                                      cfg_mode, cfg_cond, cfg_scale, include_shape=True) # coef 输出  dict_keys(['exp', 'pose', 'shape'])
        # exp [rep_n,frame_num,50] pose [rep_n,frame_num,6] shape [rep_n,frame_num,100]
        # shape还能动，但是exp暂时不能动
        ...
        try:
            ...
            verts_list = coef_dict_to_vertices(coef_dict, self.flame, self.rot_repr,
                                               ignore_global_rot=ignore_global_rot).detach().cpu().numpy()
    ...
    def infer_coeffs(self, audio, shape_coef, style_feat=None, n_repetitions=1,
        ...
        if style_feat is not None:
            ...
            style_feat = style_feat.unsqueeze(0).expand(n_repetitions, -1) # [3,128]
        ...
        # Prepare audio input, 对不足的音频进行填充
        ...
        if not self.no_context_audio_feat:
            audio_feat = self.model.extract_audio_feature(audio.unsqueeze(0), self.n_motions * n_subdivision) # self.n_motions * n_subdivision即frame_num
            # audio_feat (1, 400, 512)
        ...
        coef_list = []
        for i in range(0, n_subdivision): # 分段处理
            start_idx = i * stride
            ...
            # motion_feat [3,100,54], prev_audio_feat[3,100,512]
            else:
                motion_feat, noise, prev_audio_feat = self.model.sample(audio_in, shape_coef, style_feat,
        ...
        motion_coef = torch.cat(coef_list, dim=1) # [3, 333, 54]
    ...
    def infer_vertices(self, audio_path, coef_path, style_path=None, n_repetitions=1, ignore_global_rot=False,
        ...
        try:
            ...
            vert_list = coef_dict_to_vertices(coef_dict, self.flame, self.rot_repr,
                                              ignore_global_rot=ignore_global_rot)
        return vert_list
    ...
    def render_to_images(self, verts_list, texture=None):
        """
        Args:
            verts_list (np.ndarray): (L, 5023, 3)
        """
        ...
        rendered_frames = np.array(rendered_frames)  # 将列表转换为numpy数组以加速
        rendered_tensor = torch.tensor(rendered_frames)
        return rendered_tensor


    def infer_from_file_audio(self, audio_path, coef_path, out_path, style_path=None, tex_path=None, n_repetitions=1,
                        ignore_global_rot=False, cfg_mode=None, cfg_cond=None, cfg_scale=1.15):
        coef_dict = self.infer_coeffs(audio_path, coef_path, style_path, n_repetitions,
                                      cfg_mode, cfg_cond, cfg_scale, include_shape=True) # coef 输出  dict_keys(['exp', 'pose', 'shape'])
        # exp [rep_n,frame_num,50] pose [rep_n,frame_num,6] shape [rep_n,frame_num,100]
        ...
        if n_repetitions == 1:
            if self.save_coef:
                self.save_coef_file({k: v[0] for k, v in coef_dict.items()}, out_path.with_suffix('.npz'))
            rendered_images = self.render_to_images(verts_list[0], tex_path)
        else:
            all_rendered_images = []
            for i, verts in enumerate(verts_list):
                out_path_i = out_path.parent / f'{out_path.stem}_{i:03d}{out_path.suffix}'
                if self.save_coef:
                    self.save_coef_file({k: v[i] for k, v in coef_dict.items()}, out_path_i.with_suffix('.npz'))
                rendered_images = self.render_to_images(verts, tex_path)
                all_rendered_images.append(rendered_images.unsqueeze(0))
            rendered_images = torch.cat(all_rendered_images, dim=0)
