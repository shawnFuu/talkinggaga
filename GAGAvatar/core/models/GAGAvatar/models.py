...
def save_points_to_ply(points, save_dir, filename, k=5023):
    import os
    import numpy as np
    from plyfile import PlyElement, PlyData
    os.makedirs(save_dir, exist_ok=True)
    points = points[0].detach().cpu().numpy()  # [N,3] 
    vertices = np.zeros(len(points), dtype=[('x', 'f4'), ('y', 'f4'), ('z', 'f4'), ('red', 'u1'), ('green', 'u1'), ('blue', 'u1')])
    vertices['x'] = points[:, 0]
    vertices['y'] = points[:, 1]
    vertices['z'] = points[:, 2]
    # 对前k个点用不一样的着色
...
class GAGAvatar(nn.Module):
    def __init__(self, model_cfg=None, **kwargs):
        ...
        self.harmo_encoder = HarmonicEmbedding(n_harmonic_dir) # 方向编码/位置编码
        # pre_trained
        ...
        self.upsampler = StyleUNet(in_size=512, in_dim=32, out_dim=3, out_size=512) # Neural Renderer
    ...
    def forward_expression(self, batch):
        if not hasattr(self, '_gs_params'):
            batch_size = batch['f_image'].shape[0]
            f_image, f_planes = batch['f_image'], batch['f_planes'] # f_image [1,3,518,518] f_planes[1, 87616=296*296, 3]
            f_feature0, f_feature1 = self.base_model(f_image) # DINO提取特征 [1, 256, 296, 296] [1, 768]
            # dir encoding
            plane_direnc = self.harmo_encoder(f_planes['plane_dirs']) # 方向编码 [1, 27] f_planes: 'plane_points', 'plane_dirs'
            # global part
            gs_params_g = self.gs_generator_g( # color [1, 5023, 32] 
                torch.cat([
                        self.head_base[None].expand(batch_size, -1, -1), f_feature1[:, None].expand(-1, 5023, -1), 
                    ], dim=-1
                ), plane_direnc # self.head_base [5023, 256] -> [1, 5023, 256] -> [1, 5023, 256]; f_feature1 [1, 768] -> [1,1,768] ->(复制) [1, 5023, 768]
            )
            gs_params_g['xyz'] = batch['f_image'].new_zeros((batch_size, 5023, 3))
            # local part
            gs_params_l0 = self.gs_generator_l0(f_feature0, plane_direnc) # color [1,87616,32] 高斯数就是f_planes的特征点数
            gs_params_l1 = self.gs_generator_l1(f_feature0, plane_direnc)
            # plane_points是像素点吗？
            gs_params_l0['xyz'] = f_planes['plane_points'] + gs_params_l0['positions'] * f_planes['plane_dirs'][:, None] # 向前偏移 gs_params_l0['positions'] [1,87616,1], f_planes['plane_dirs'][:, None] [1,1,3]
            gs_params_l1['xyz'] = f_planes['plane_points'] + -1 * gs_params_l1['positions'] * f_planes['plane_dirs'][:, None] # 向后偏移
            gs_params = {
                k:torch.cat([gs_params_g[k], gs_params_l0[k], gs_params_l1[k]], dim=1) for k in gs_params_g.keys()
            }
            # scale = 5.0
            # gs_params['xyz'] = gs_params['xyz'] / scale  # 将gs尺度缩放到与t_points一致
            self._gs_params = gs_params # xyz [1, 5023 + 87616 * 2 = 180255, 3]
        gs_params = self._gs_params

        # ---audio-driven begin---

        t_image, t_points, t_transform = batch['t_image'], batch['t_points'], batch['t_transform'] # t_image [1, 3, 512, 512]
        # t_image没有，t_points就是prediction[i],t_transform也没有，但是可以用feature_data['transform_matrix']

        gs_params['xyz'][:, :5023] = t_points # (audio)只需要替换t_points; Q:可能导致推理图像多余的部分直接粘贴
        # 驱动图像只起到提供t_points和t_transform的作用

        # frame_idx = str(batch["infos"]["t_key"]).split('[')[1].split(']')[0]
        # # 合并t_points和gs_params['xyz']
        # save_points_to_ply(
        #     gs_params['xyz'],
        #     "/data/fuxiaowen/gaga/trying_demo/gs_points_driven",
        #     f'gs_points_driven_{frame_idx}.ply'
        # )

        # ---audio-driven end---

        gen_images = render_gaussian( # [1, 32, 512, 512] 颜色32维
            gs_params=gs_params, cam_matrix=t_transform, cam_params=self.cam_params # 这一步提供相机参数
        )['images']
        sr_gen_images = self.upsampler(gen_images) # [1, 3, 512, 512]
        results = {
    ...
    def forward_expression_audio(self, batch):
        if not hasattr(self, '_gs_params'):
            batch_size = batch['f_image'].shape[0]
            f_image, f_planes = batch['f_image'], batch['f_planes'] # f_image [1,3,518,518] f_planes[1, 87616=296*296, 3]
            f_feature0, f_feature1 = self.base_model(f_image) # DINO提取特征 [1, 256, 296, 296] [1, 768]
